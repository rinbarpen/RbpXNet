...
def get_dwconv(dim, kernel=7, bias=False):
    return nn.Conv2d(
        dim, dim, kernel_size=kernel, padding=kernel // 2, bias=bias, groups=dim
    )
...
def get_dwconv_layer2d(
    ...
    return nn.Sequential(
        depth_conv, nn.ReLU(inplace=True), point_conv, nn.ReLU(inplace=True)
    )


def get_dwconv_layer3d(
    in_channels: int,
    out_channels: int,
    kernel_size: int = 3,
    stride: int = 1,
